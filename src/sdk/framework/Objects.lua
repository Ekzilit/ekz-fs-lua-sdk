...
local function validateElementValueType(elementAttributes, value)
    ...
    elseif elementAttributes.type ~= nil and globEnv[elementAttributes.type] == nil then
...
local function finishClassDeclaration(rawClass, name)
    ...
    if child.classAttributes.inner == true then
        ...
        globEnv[child.package .. "." .. name] = rawClass
    else
        globEnv[child.package .. "." .. name] = rawClass
        print(child.package .. "." .. name)
        _package = nil
        _import = {}
    end
    return rawClass
    ...
    elseif rawClass.classAttributes.interface == false and parentClass.class.classAttributes.interface == true then
        ...
        if rawClass.interfaces then
            for i, interface in ipairs(rawClass.interfaces) do
                local interfaceDeclaration = globEnv[getFullPathFromImports(interface)]
...
function declareClass(name, parents, rawClass, modifiers, interfacesToImplement)
    ...
    if parents == nil or #parents == 0 then
        if name == "Object" then
            ...
            return finishClassDeclaration(extend(rawClass, globEnv["sdk.Object"]), name)
        end
    else
        for _, parent in ipairs(parents) do
...
globEnv["import"] = function(fullClassName)
    local className
...
globEnv["def"] = function(className)
    local fullClassName
    ...
    if fullClassName then
        local wrappedCalledClass = globEnv[fullClassName]
